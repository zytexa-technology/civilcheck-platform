...
const RISK_DOT = { DISPUTED: 'var(--red)', CLEAR: 'var(--green)' }
const RISK_LABEL = { CLEAR: 'Clear', DISPUTED: 'Disputed' }
const RiskDot = ({ risk }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
    <span style={{ width: 7, height: 7, borderRadius: '50%', background: RISK_DOT[risk] || 'var(--muted)', flexShrink: 0 }} />
    {RISK_LABEL[risk] || 'Unclassified'}
  </span>
)
...
export default function AlertSubs() {
  ...
  const columns = [
      ...
      render: (a) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--blue-dim)', display: 'grid', placeItems: 'center', fontWeight: 700, fontSize: 12, color: 'var(--blue)', flexShrink: 0 }}>
            {(a.buyer?.name || a.buyer?.phone || 'U')[0].toUpperCase()}
          </div>
          <div>
    ...
    { key: 'risk', header: 'Property status', render: (a) => (a.listing?.propertyStatus ? <RiskDot risk={a.listing.propertyStatus} /> : '—') },
  ...
  return (
    <div>
      <PageHead title="Alert subscriptions" subtitle="Users watching a property for free case-update notifications" />
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <SearchInput value={search} onChange={setSearch} placeholder="Filter this page by user or property…" style={{ flex: 1, minWidth: 220 }} />
