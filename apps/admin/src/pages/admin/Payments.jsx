...
export default function Payments() {
  ...
  const columns = [
    ...
    { key: 'buyer', header: 'User', render: (t) => (<><div style={{ fontWeight: 600 }}>{t.buyerName}</div><div className="small muted">{t.buyerPhone}</div></>) },
    ...
    { key: 'sellerCut', header: 'Partner cut', render: (t) => <span style={{ color: 'var(--green)' }}>₹{t.sellerCut.toLocaleString('en-IN')}</span> },
    ...
    <div>
      ...
      {data && (
        <div className="grid g4" style={{ marginBottom: 20 }}>
          ...
          <StatCard tone="violet" icon="🤝" value={`₹${data.summary.sellerPayouts.toLocaleString('en-IN')}`} label="Partner payouts" />
        </div>
      )}
