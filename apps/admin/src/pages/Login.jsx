  ...
  return (
    <div style={s.bg}>
      ...
      <div style={s.box}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20, borderBottom: '1px solid #e4e7ec' }}>
          {['password', 'phone'].map(m => (
            <button
              ...
              style={{
                ...
                borderBottom: method === m ? '2px solid #f0a500' : '2px solid transparent',
                color: method === m ? '#12141c' : '#5b6472',
        ...
        {method === 'password' ? (
          <>
            ...
            <div style={s.field}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => setShowPass(!showPass)}
                  style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#5b6472', cursor: 'pointer', fontSize: 16, padding: 4 }}>
                  {showPass ? '🙈' : '👁️'}
                </button>
              </div>
            </div>
            ...
            {!totpStage && (
              <div style={{ textAlign: 'right', marginTop: -8, marginBottom: 16 }}>
                <Link to="/forgot-password" style={{ fontSize: 12.5, color: '#5b6472' }}>
                  Forgot password?
                </Link>
              </div>
            )}
            ...
            <button className="login-btn" onClick={handleLogin} disabled={loading} style={{ ...s.btn, opacity: loading ? 0.7 : 1 }}>
              {loading ? (
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  <span style={{ width: 14, height: 14, border: '2px solid rgba(26,18,0,.3)', borderTop: '2px solid #1a1200', borderRadius: '50%', animation: 'spin .7s linear infinite', display: 'inline-block' }} />
                  Logging in...
                </span>
          ...
          <div style={{ fontSize: 12.5, color: '#5b6472', lineHeight: 1.6, marginBottom: 8 }}>
            Phone-OTP login is not available on this deployment yet — use email and password.
          </div>
        ) : (
          <>
            <div id={PHONE_OTP_RECAPTCHA_ID} />
            <div style={{ fontSize: 11.5, color: '#5b6472', lineHeight: 1.6, marginBottom: 14 }}>
            ...
            {otpStage === 'phone' ? (
              ...
              <>
                <div style={{ fontSize: 12, color: '#5b6472', marginBottom: 12 }}>
                  ...
                  <button onClick={() => { setOtpStage('phone'); setOtpCode(''); setError('') }} style={{ background: 'none', border: 'none', color: '#2563eb', cursor: 'pointer', font: 'inherit' }}>
                    Change number
                  </button>
                </div>
                <div style={s.field}>
